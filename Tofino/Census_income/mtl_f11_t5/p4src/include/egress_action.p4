action task6_predict(){
    eg_md.meta.test = (bit<8>)(task6_prediction.execute(0));
}

action task12_predict(){
    eg_md.meta.test = (bit<8>)(task12_prediction.execute(0));
}

action task10_predict(){
    eg_md.meta.test = (bit<8>)(task10_prediction.execute(0));
}

action task2_predict(){
    eg_md.meta.test = (bit<8>)(task2_prediction.execute(0));
}

action task13_predict(){
    eg_md.meta.test = (bit<8>)(task13_prediction.execute(0));
}

action reset_register_task6(){
    task6_reset.execute(0);
}
action reset_register_task12(){
    task12_reset.execute(0);
}
action reset_register_task10(){
    task10_reset.execute(0);
}
action reset_register_task2(){
    task2_reset.execute(0);
}
action reset_register_task13(){
    task13_reset.execute(0);
}

action invalid_header(){
    hdr.intermediate_data1.setInvalid();
    hdr.intermediate_data2.setInvalid();
    hdr.maloi.layer = 0;
}

#define ACTIVATE_ONE(i,j)\
action activate##i##_##j##_one(){\
    hdr.intermediate_data##i##.intermediate_data_##j## = hdr.intermediate_data##i##.intermediate_data_##j## + 1;\
}\

#define ACTIVATE_ZERO(i,j)\
action activate##i##_##j##_zero(){\
    hdr.intermediate_data##i##.intermediate_data_##j## = hdr.intermediate_data##i##.intermediate_data_##j## << 1;\
}\

#define PREDICT(i, j)\
action real_task##i##_predict(bit<8> x){\
    hdr.labeling_##j##.prediction##i## = x;\
}\

ACTIVATE_ONE(1,1)
ACTIVATE_ONE(1,2)
ACTIVATE_ONE(1,3)
ACTIVATE_ONE(1,4)
ACTIVATE_ONE(1,5)

ACTIVATE_ONE(2,1)
ACTIVATE_ONE(2,2)
ACTIVATE_ONE(2,3)
ACTIVATE_ONE(2,4)
ACTIVATE_ONE(2,5)

ACTIVATE_ZERO(1,1)
ACTIVATE_ZERO(1,2)
ACTIVATE_ZERO(1,3)
ACTIVATE_ZERO(1,4)
ACTIVATE_ZERO(1,5)

ACTIVATE_ZERO(2,1)
ACTIVATE_ZERO(2,2)
ACTIVATE_ZERO(2,3)
ACTIVATE_ZERO(2,4)
ACTIVATE_ZERO(2,5)

PREDICT(6,1)
PREDICT(12,1)
PREDICT(10,2)
PREDICT(2,2)
PREDICT(13,3)